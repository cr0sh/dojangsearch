...
function formatDate(date) {
	...
	<div id="result">
		...
		알림: 최근 랭킹의 달성 날짜가 적절하지 않은 오류가 발견되어 수정되었습니다. 그러나 이미 수집된 랭킹 DB를 수정할 수는 없으므로 2018-02-24 이전의 '최근 랭킹' 달성 날짜는 주 단위로만 정확함을 인지해 주시기 바랍니다. 2018-03-11에 2018-03-01부터 다시 수집한 DB로 교체할 예정입니다. (기존 DB도 교체 전까지 계속 업데이트됩니다)<br>
		알림: 일요일에 달성한 무릉 기록이 정상적으로 기록되지 않는 현상이 발견되어 수정 중입니다. 2018-03-01부터 수집한 새 DB는 이를 정상적으로 반영합니다. (교체 전 DB도 일부 기록을 복구하였습니다.)<br>
	</div>
</body>
</html>
`
